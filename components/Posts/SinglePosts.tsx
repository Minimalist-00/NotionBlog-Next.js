      ...
      {isSeeMorePage ? (
        <section className="bg-slate-200 text-gray-700 p-4 mb-8 rounded-lg">
          <div className="flex">
            ...
            {tags.map((tag: string, index: number) => (
              <span className="font-medium text-gray-700 bg-gray-300 px-2 rounded-xl inline-block">
                {tag}
              </span>
            ))}
          </div>
        ...
        <section className="bg-gray-600 text-gray-100 p-4 mb-8 rounded-lg">
          <div className="flex">
            <h2>{title}</h2>
            {tags.map((tag: string, index: number) => (
              <span className="font-medium text-gray-700 bg-gray-300 px-2 mx-1 rounded-xl inline-block">
                {tag}
              </span>
            ))}
          </div>
